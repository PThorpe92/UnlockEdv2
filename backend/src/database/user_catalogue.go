...
func (db *DB) GetUserCatalogue(userId int, tags []string, search, orderBy string) ([]UserCatalogueJoin, error) {
	...
	if search != "" {
		tx.Where("LOWER(p.name) LIKE ?", "%"+search+"%").Order(fmt.Sprintf("p.name %s", validOrder(orderBy)))
	}
