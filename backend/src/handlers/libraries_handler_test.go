	...
	for _, test := range httpTests {
		t.Run(test.testName, func(t *testing.T) {
			...
			if test.expectedStatusCode == http.StatusOK {
				_, expectedLibraries, err := server.Db.GetAllLibraries(test.mapKeyValues["page"].(int), test.mapKeyValues["per_page"].(int), test.mapKeyValues["user_id"].(uint), test.mapKeyValues["visibility"].(string), "created_at DESC", test.mapKeyValues["search"].(string))
				if err != nil {
